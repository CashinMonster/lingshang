...
function Enemy(){
	...
	this.imgenemyX = 0;
	this.imgenemyY = 0;
	this.x = fnRand(80,canvasWidth - this.w - 80);
			...
			for (var j = 0,lenj = enemys.length; j < lenj; j++) {
				if (enemys[j].y < canvasHeight && enemys[j].blood > 0 && crash(bullets[i],enemys[j])) {
					...
					if (enemys[j].blood <= 0) {
						...
						if (grade > 1500 && grade < 1600) {
							bulletspeed = 8;
							enemyspeed = 10;
							speed = 12;
						};
						if (grade > 3000 && grade < 3100) {
							bulletspeed = 6;
							enemyspeed = 6;
							speed = 14;
						};
						if (grade > 5000 && grade < 5100) {
							bulletspeed = 5;
							enemyspeed = 4;
							speed = 17;
						};
						if (grade > 7000 && grade < 7100) {
							bulletspeed = 5;
							enemyspeed = 4;
							speed = 20;
						};
...
var kais = true;
// 开始游戏点击事件
$(".shouye_kaishi").get(0).addEventListener("touchstart", function (e){
	musicbg.play();
	$(".shouye").hide();
	$(".guize").show();
	setTimeout(function(){
...
// 与朋友PK点击事件
$(".anniu_pk1").get(0).addEventListener("touchstart", function (){
	$("#over").hide();
	$(".game1").hide();
	$(".game2").hide();
	$(".game3").hide();
...
$(".anniu_pk2").get(0).addEventListener("touchstart", function (){
	$("#over").hide();
	$(".game1").hide();
	$(".game2").hide();
	$(".game3").hide();
